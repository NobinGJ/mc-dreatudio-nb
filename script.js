...
// Iniciar música automáticamente apenas se pueda
const playMusic = () => {
    if (!isMuted) {
        bgMusic.play().catch(e => {
            console.log("El autoplay fue prevenido por el navegador. Se iniciará al hacer clic en la pantalla:", e);
            // Si el navegador lo bloquea, ponemos un fallback para el primer clic
            document.body.addEventListener('click', () => {
                if (!isMuted && bgMusic.paused) {
                    bgMusic.play().catch(err => console.log("Autoplay err:", err));
                }
            }, { once: true });
        });
    }
...
// Intentar iniciar la música directamente en cuanto carga todo el script
playMusic();
